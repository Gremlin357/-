...
function addTerritory(path, definition) {
  ...
  if (definition.color === 0x59a37e) forestShapes.push(...shapes);
}

function addHouse(path) {
  const bodyMaterial = new THREE.MeshStandardMaterial({ color: 0xf8f8f2, roughness: 0.78 });
  const roofMaterial = new THREE.MeshStandardMaterial({ color: 0x535755, roughness: 0.76 });
  SVGLoader.createShapes(path).forEach((shape) => {
    const body = extrudeShape(shape, 2.05, bodyMaterial);
    body.position.y = layers["#CD80DD"].elevation + layers["#CD80DD"].height + 0.02;
    map.add(body);
    const roof = extrudeShape(shape, 0.34, roofMaterial);
    roof.position.y = body.position.y + 2.07;
    map.add(roof);
  });
}

function pointInAnyForest(point) {
  return forestShapes.some((shape) => shape.containsPoint(point));
}
...
function addTree(x, z, scale) {
  ...
  const crown = new THREE.Mesh(new THREE.ConeGeometry(0.68, 1.55, 9), treeCrowns[Math.round(x * 7 + z * 3) & 1]);
...
async function buildModel() {
  ...
  const parsed = new SVGLoader().parse(await response.text());
  parsed.paths.forEach((path) => {
    const fill = fillOf(path);
    if (layers[fill]) addTerritory(path, layers[fill]);
  });
  parsed.paths.forEach((path) => {
    if (fillOf(path) === houseFill) addHouse(path);
  });
  seedTrees();
